...
const DailyNewsPage = () => {
  ...
  // Get all articles for this date, grouped by topic
  const dateArticles = allArticles.filter(article => {
    // Match articles by date string
  ...
  const articlesByTopic = dateArticles.reduce((acc, article) => {
    if (!acc[article.topic]) {
      acc[article.topic] = [];
    }
    acc[article.topic].push(article);
    return acc;
  }, {} as Record<string, typeof dateArticles>);
  ...
  return (
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="secondary" className="text-sm">
              {dateArticles.length} Articles • {topics.length} Topics
            </Badge>
          </div>
          <div className="mt-3">
      ...
      {/* Topic Navigation */}
      <div className="bg-muted/50 border-b sticky top-[88px] z-10">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        <div className="space-y-10">
          {topics.map((topic, topicIndex) => (
            <div key={topic} id={`topic-${topic}`} className="scroll-mt-32">
              {/* Topic Header */}
