...
const SyllabusPage = () => {
  ...
  const [resourceDialog, setResourceDialog] = useState<{
    ...
    subjectName: string;
  }>({ isOpen: false, topic: null, subjectName: '' });
  ...
  const overallStats = useMemo(() => {
    ...
    return {
      completed: Math.round(totalProgress / totalTopics),
      totalTopics,
      avgScore: 72,
      totalVideos,
      totalQuestions
      ...
      <div className="p-6">
        <Card className="p-8 text-center">
          <p className="text-muted-foreground">
            No syllabus available for the selected exam categories.
            Please select exam categories from your profile.
          </p>
        </Card>
      </div>
    );
  ...
  return (
    <div className="p-4 md:p-6 space-y-4 max-w-7xl mx-auto">
        ...
        {availableExams.map((exam) => (
          ...
          >
            ...
            <div
              ...
              className={`ml-1 w-4 h-4 rounded border flex items-center justify-center cursor-pointer ${
                ...
                <div className="border-t bg-muted/20 p-4">
                  <div className="grid gap-2">
                    {subject.topics.map((topic) => (
                      <div
                        key={topic.id}
                        className="flex items-center gap-3 p-3 bg-background rounded-lg hover:bg-muted/30 transition-colors cursor-pointer"
                        onClick={() => setResourceDialog({ isOpen: true, topic, subjectName: subject.name })}
                      >
                        <div className="flex-1">
                          <p className="font-medium text-sm">{topic.name}</p>
                          <div className="flex items-center gap-2 mt-1">
                            <Progress value={topic.progress} className="h-1 flex-1 max-w-24" />
      ...
      <Dialog open={resourceDialog.isOpen} onOpenChange={(open) => setResourceDialog({ isOpen: open, topic: null, subjectName: '' })}>
          ...
          {resourceDialog.topic && (
            <Tabs defaultValue="videos" className="mt-4">
              ...
              <div className="mt-4 space-y-3">
                {/* Videos Tab Content */}
                <div className="data-[state=active]:block data-[state=inactive]:hidden" data-state="active">
                  ...
                  {videoPlayer.playlist.map((video, index) => (
                    <button
                      ...
                      className={`w-full p-2 rounded-lg text-left transition-colors ${
                        video.id === videoPlayer.video?.id ? 'bg-emerald-100 text-emerald-900' : 'hover:bg-muted'
                      }`}
                    >
                      <div className="flex items-center gap-2">
