...
const ProfileHeader: React.FC<ProfileHeaderProps> = ({ onToggleSidebar }) => {
  ...
  const navItems = [
    ...
    { icon: <Lightbulb size={18} />, label: 'Speed Drills', to: '/student/speed-drills' },
                    ...
                    <ul className="space-y-1 sm:space-y-2">
                      {navItems.map((item) => (
                        <li key={item.to}>
                          <Link 
                            to={item.to}
                            className={`flex items-center gap-2 sm:gap-3 rounded-lg px-2 sm:px-3 py-2 sm:py-3 text-sm transition-all hover:bg-gray-100 touch-manipulation ${
