      ...
      <DialogContent className={`${isMobile ? 'max-w-[95vw] max-h-[95vh] m-2' : 'max-w-4xl max-h-[90vh]'} overflow-y-auto p-0`}>
        <div className={isMobile ? 'p-4' : 'p-6'}>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className={`${isMobile ? 'grid w-full grid-cols-2' : 'grid w-full grid-cols-5'} mb-6`}>
              ...
              <TabsTrigger value="packages" className={isMobile ? 'text-xs' : 'text-sm'}>Packages</TabsTrigger>
            ...
            <TabsContent value="packages" className="space-y-4">
                ...
                {packages.map((pkg) => (
                  ...
                  >
                    <CardHeader className="text-center">
                      <CardTitle className={isMobile ? 'text-base' : 'text-lg'}>{pkg.name}</CardTitle>
                      <div className={`font-bold text-blue-600 ${isMobile ? 'text-2xl' : 'text-3xl'}`}>₹{pkg.price.toLocaleString()}</div>
                      <p className={`text-gray-600 ${isMobile ? 'text-xs' : 'text-sm'}`}>{pkg.sessions} sessions • {pkg.duration}</p>
                    </CardHeader>
                    <CardContent>
                      <ul className="space-y-2">
                        {pkg.features.map((feature, index) => (
                          <li key={index} className={`flex items-center ${isMobile ? 'text-xs' : 'text-sm'}`}>
              ...
              {selectedPackage && (
                <div className="text-center">
                  <Button size="lg" className={`bg-blue-600 hover:bg-blue-700 ${isMobile ? 'w-full touch-button' : ''}`}>
                    Book {packages.find(p => p.id === selectedPackage)?.name} Package
                  </Button>
                </div>
              )}
            </TabsContent>
